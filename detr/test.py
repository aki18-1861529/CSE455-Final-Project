...
def infer(images_path, model, postprocessors, device, output_path):
    ...
    for img_sample in images_path:
        ...
        for idx, box in enumerate(bboxes_scaled):
            ...
            print(bbox)
            cropped_image = img[(bbox[0][1]):(bbox[2][1]), (bbox[0][0]):(bbox[2][0])]
            '''if (bbox[0][1]-10 < 0 or bbox[2][1]+10 > imh or bbox[0][0]-10 < 0 or bbox[2][0]+10 > imw):
                cropped_image = img[(bbox[0][1]):(bbox[2][1]), (bbox[0][0]):(bbox[2][0])]
            else:
                cropped_image = img[(bbox[0][1]-10):(bbox[2][1]+10), (bbox[0][0]-10):(bbox[2][0]+10)]
            '''
            
            # cv2_imshow(cropped_image)
            img_save_path = os.path.join(output_path, filename+"_"+str(idx))
            cv2.imwrite(img_save_path, cropped_image)
#             [[294 103]
            #  [371 103]
            #  [371 200]
            #  [294 200]]
